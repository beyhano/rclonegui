...
pub async fn execute_task(
    ...
    // Read stdout (progress parsing happens, but we just check for errors)
    let mut stdout_lines = BufReader::new(stdout).lines();
    while let Ok(Some(_line)) = stdout_lines.next_line().await {
        // Progress lines are handled — we consume stdout to avoid pipe deadlocks
    }
...
mod tests {
    ...
    async fn test_execute_task_invalid_path_returns_error() {
        ...
        let result = execute_task(&task, "/nonexistent/rclone").await;
